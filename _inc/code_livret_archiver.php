...
if($action=='imprimer_donnees_eleves_prof')
{
  ...
  foreach($tab_saisie as $rubrique_id => $tab)
  {
    if(isset($tab_rubriques[$rubrique_id]))
    {
      ...
      $archivage_tableau_PDF->appreciation_intitule( 'Livret scolaire - '.$classe_nom.' - '.$periode_nom.' - Appréciations de '.$prof_nom.' - '.$tab_rubriques[$rubrique_id] );
...
if($action=='imprimer_donnees_eleves_collegues')
{
  ...
  foreach($DB_TAB as $DB_ROW)
  {
    if(!isset($tab_saisie[$DB_ROW['eleve_id']][$DB_ROW['rubrique_id']]))
    {
      ...
      $note = ( ($PAGE_REF!='brevet') && ($PAGE_COLONNE!='rien') ) ? $DB_ROW['saisie_note'] : NULL ; // TODO : enlever le test "rien" si ce n'est pas autorisé (pour l'instant ce n'est même pas implémenté...)
...
if($action=='imprimer_donnees_classe_collegues')
{
  ...
  foreach($DB_TAB as $DB_ROW)
  {
    if(!isset($tab_saisie[$DB_ROW['rubrique_id']]))
    {
      // Initialisation, dont la note pour le bulletin
      $rubrique_nom = ($DB_ROW['rubrique_nom']!==NULL) ? $DB_ROW['rubrique_nom'] : 'Synthèse générale' ;
      $note = ( ($PAGE_REF!='brevet') && ($PAGE_COLONNE!='rien') && $_SESSION['OFFICIEL']['BULLETIN_MOYENNE_CLASSE'] ) ? $DB_ROW['saisie_note'] : NULL ; // TODO : enlever le test "rien" si ce n'est pas autorisé (pour l'instant ce n'est même pas implémenté...)
