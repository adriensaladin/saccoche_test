...
if( !$page_ref || !$groupe_id || !$matiere_id || !$prof_id || !$titre || !DB_STRUCTURE_LIVRET::DB_tester_page_avec_dispositif( $page_ref , 'ap' ) )
{
...
if( ($action=='ajouter') || ($action=='dupliquer') )
{
  ...
  if( DB_STRUCTURE_LIVRET::DB_tester_ap( $page_ref , $groupe_id , $matiere_id , $prof_id ) )
  {
    Json::end( FALSE , 'A.P. classe / enseignant / matière déjà existant !' );
  }
  // Insérer l'enregistrement
  $ap_id = DB_STRUCTURE_LIVRET::DB_ajouter_ap( $page_ref , $groupe_id , $matiere_id , $prof_id , $titre );
  // Afficher le retour
  ...
  Json::add_str(  '<td data-id="'.$matiere_id.'">{{MATIERE_NOM}}</td>');
...
{
  // Vérifier que l'accompagnement personnalisé est disponible
  if( DB_STRUCTURE_LIVRET::DB_tester_ap( $page_ref , $groupe_id , $matiere_id , $prof_id , $ap_id ) )
  {
    Json::end( FALSE , 'A.P. classe / enseignant / matière déjà existant !' );
  }
  // Mettre à jour l'enregistrement
  // Remarque : il est possible qu'il n'y ait aucun changement, on ne s'en préoccupe pas.
  DB_STRUCTURE_LIVRET::DB_modifier_ap( $ap_id , $page_ref , $groupe_id , $matiere_id , $prof_id , $titre );
  // Afficher le retour
  Json::add_str('<td data-id="'.$page_ref.'">{{PAGE_MOMENT}}</td>');
  Json::add_str('<td data-id="'.$groupe_id.'">{{GROUPE_NOM}}</td>');
  Json::add_str('<td data-id="'.$matiere_id.'">{{MATIERE_NOM}}</td>');
